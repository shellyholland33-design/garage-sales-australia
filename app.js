...
qs("#csv_input").addEventListener("change", async e => {
  ...
  all_sales = rows.map((line, i) => {
    ...
    return {
      ...
      suburb: c[idx("suburb")] || "",
      postcode: (c[idx("postcode")] || "").trim(),
      date: (c[idx("date")] || "").trim(),
      start_time: c[idx("start_time")] || "",
      end_time: c[idx("end_time")] || "",
      description: c[idx("description")] || "",
      lat: Number(c[idx("lat")]) || null,
      lng: Number(c[idx("lng")]) || null,
      url: c[idx("url")] || ""
    };
  });
  apply_filters();
...
function apply_filters() {
  ...
  let list = all_sales.slice().filter(it => {
    if (kw) {
    ...
    if (radius_km && origin) {
      ...
      const dist = p ? km_between(origin, p) : null;
  ...
  list.forEach(it => it._distance_km = o ? km_between(o, point_for_sale(it)) : null);

  const sort = qs("#sort_by").value;
  list.sort((a, b) => {
    if (sort === "distance_asc") return (a._distance_km ?? 999999) - (b._distance_km ?? 999999);
...
function render_cards(list) {
  ...
  const tmpl = document.getElementById("card_template");
  list.forEach(it => {
    if (dismissed[it.id]) return;
    const frag = tmpl.content.cloneNode(true);
    const root = frag.querySelector(".card");

    frag.querySelector(".card_title").textContent = it.title || "Garage sale";
    frag.querySelector(".chip_date").textContent = pretty_date(it);
    frag.querySelector(".card_addr").textContent = address_line(it);
    frag.querySelector(".card_desc").textContent = it.description || "";
    const dist_line = frag.querySelector(".card_dist");
    if (it._distance_km != null) { dist_line.hidden = false; dist_line.textContent = `${it._distance_km} km from origin`; }

    const link = frag.querySelector(".btn_visit");
    link.href = it.url || gmaps_link(it);

    const btn_viewed = frag.querySelector(".btn_viewed");
    ...
    const btn_dismiss = frag.querySelector(".btn_dismiss");
...
function address_line(it) { return [it.address, it.suburb, it.postcode].filter(Boolean).join(", "); }
function gmaps_link(it) { const q = encodeURIComponent(`${it.address || ""} ${it.suburb || ""} ${it.postcode || ""}`); return `https://maps.google.com/?q=${q}`; }

/* PWA install */
let deferred;
window.addEventListener("beforeinstallprompt", e => { e.preventDefault(); deferred = e; qs("#install_btn").hidden = false; });
qs("#install_btn").addEventListener("click", async () => { if (!deferred) return; deferred.prompt(); await deferred.userChoice; deferred = null; qs("#install_btn").hidden = true; });
